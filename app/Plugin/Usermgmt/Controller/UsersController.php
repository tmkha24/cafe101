		...
		if ($this->request -> isPost()) {
			...
			if($this->User->LoginValidate()) {
				...
				if (empty($user)) {
					...
					if (empty($user)) {
						$this->Session->setFlash(__('Incorrect Email/Username or Password'),'default', array("class"=>"alert alert-warning "));
						return;
					}
				}
				// check for inactive account
				if ($user['User']['id'] != 1 and $user['User']['active']==0) {
					$this->Session->setFlash(__('Your registration has not been confirmed please verify your email or contact to Administrator'),'default', array("class"=>"alert alert-info "));
					return;
				}
				$hashed = md5($password);
			...
			if (!empty($user)) {
				if (!$user['User']['active']) {
					...
					if ($activateKey==$theKey) {
						...
						$this->Session->setFlash(__('Thank you, your account is activated now'),'default', array("class"=>"alert alert-success "));
					}
				} else {
					$this->Session->setFlash(__('Thank you, your account is already activated'),'default', array("class"=>"alert alert-success "));
				}
			} else {
				$this->Session->setFlash(__('Sorry something went wrong, please click on the link again'),'default', array("class"=>"alert alert-warning "));
			}
		} else {
			$this->Session->setFlash(__('Sorry something went wrong, please click on the link again'),'default', array("class"=>"alert alert-warning "));
		}
	...
	public function forgotPassword() {
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->LoginValidate()) {
				...
				// check for inactive account
				if ($user['User']['id'] != 1 and $user['User']['active']==0) {
					$this->Session->setFlash(__('Your registration has not been confirmed yet please verify your email before reset password'),'default', array("class"=>"alert alert-info "));
					return;
				}
				$this->User->forgotPassword($user);
				$this->Session->setFlash(__('Please check your mail for reset your password'),'default', array("class"=>"alert alert-success "));
				$this->redirect('/login');
			}
		}
	}
