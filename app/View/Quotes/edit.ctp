...
<section class="content-header">
	<div class="container-fluid">
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1><?php echo __('Edit Quote');?></h1>
			</div>
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					<li class="breadcrumb-item"><a href="<?php echo Router::url('/', true)?>"><?php echo __('Home') ?></a></li>
					<li class="breadcrumb-item active"><?php echo __('Edit Quote');?></li>
				</ol>
			</div>
		</div>
	</div>
						...
						<fieldset>
							...
							<div class="form-group">
								...
								<div class="input-group date" id="reservationdate" data-target-input="nearest">
									<input value="<?php echo $quote['Quote']['date_expired']; ?>" type="text" name="data[Quote][date_expired]" class="form-control datetimepicker-input" data-target="#reservationdate"/>
				...
				<div class="card">
					...
					<div class="card-body">
						...
						<table class="table table-striped" id="service-table">
							...
							<tbody>
							<?php $i=0; foreach ($quoteItems as $item): $i++; $quoteItem = $item['QuoteItem'];?>
								<tr>
									<td><?php echo $i ?></td>
									<td><input value="<?php echo $quoteItem['name'] ?>" type="text" class="form-control s_name" id="s_name"
											   name="data[Quote][items][<?php echo $quoteItem['product_id'] ?>][name]" data-id="<?php echo $quoteItem['product_id'] ?>"></td>
									<td><input value="<?php echo $quoteItem['qty'] ?>"  type="text" class="form-control s_qty" id="s_qty"
											   name="data[Quote][items][<?php echo $quoteItem['product_id'] ?>][qty]" data-id="<?php echo $quoteItem['product_id'] ?>"></td>
									<td><input value="<?php echo $quoteItem['list_price'] ?>"  type="text" class="form-control s_list_price" id="s_list_price"
											   name="data[Quote][items][<?php echo $quoteItem['product_id'] ?>][list_price]" data-id="<?php echo $quoteItem['product_id'] ?>"></td>
									<td><input value="<?php echo $quoteItem['unit_price'] ?>" type="text" class="form-control s_unit_price" id="s_unit_price"
											   name="data[Quote][items][<?php echo $quoteItem['product_id'] ?>][unit_price]" data-id="<?php echo $quoteItem['product_id'] ?>"></td>
									<td><input value="<?php echo $quoteItem['amount'] ?>" type="text" class="form-control s_amount" id="s_amount"
											   name="data[Quote][items][<?php echo $quoteItem['product_id'] ?>][amount]" data-id="<?php echo $quoteItem['product_id'] ?>"></td>
						...
						<div class="table-responsive">
							<table class="table">
								<tbody><tr>
									...
									<td><input  value="<?php echo $quote['Quote']['amount'] ;?>" readonly="true" name="data[Quote][amount]" class="form-control"  type="number" id="subtotal"></td>
								</tr>
								<tr>
									<th><?php echo __('Discount Amount:');?></th>
									<td style="vertical-align: inherit"><span style="font-weight: bold;" class="symbol-currency"></span></td>
									<td><input value="<?php echo $quote['Quote']['discount_amount'] ;?>" name="data[Quote][discount_amount]" data-type="currency" class="form-control" step="1" type="number" id="quotediscount"></td>
								</tr>
								<tr>
									<th><?php echo __('Shipping Cost:');?></th>
									<td style="vertical-align: inherit"><span style="font-weight: bold;" class="symbol-currency"></span></td>
									<td><input  value="<?php echo $quote['Quote']['shipping_cost'] ;?>" name="data[Quote][shipping_cost]"  class="form-control" step="1" type="number" id="quoteshippingcost"></td>
								</tr>
								<tr>
									<th><?php echo __('Total:');?></th>
									<td style="vertical-align: inherit"><span style="font-weight: bold;" class="symbol-currency"></span></td>
									<td><input  value="<?php echo $quote['Quote']['grant_total'] ;?>"  readonly="true" name="data[Quote][grant_total]" class="form-control" step="1" type="number" id="total"></td>
								</tr>
								</tbody></table>
						</div>
					</div>
...
<script>
	...
	$( document ).ready(function() {
		if ($('#Quotesame_as_billing_address').is(':checked')) {
			$('.shipping_address').attr('disabled','disabled'); //enable input
			$('#QuoteShippingContactName').val($('#QuoteBillingContactName').val());
			$('#QuoteShippingContactEmail').val($('#QuoteBillingContactEmail').val());
			$('#QuoteShippingContactPhone').val($('#QuoteBillingContactPhone').val());
